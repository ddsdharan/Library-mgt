...
function BorrowBook() {
    ...
    let getUsers = async () => {
        try {
            const details = await axios.get(`https://65a11ecc600f49256fb0f56b.mockapi.io/books/${params.id}`);
    ...
    const myFormik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                setLoading(true);
                await axios.put(`https://65a11ecc600f49256fb0f56b.mockapi.io/books/${params.id}`, values);
            } catch (error) {
                console.log(error);
                setLoading(false);
            }
